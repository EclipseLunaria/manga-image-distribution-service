...
import {
  checkChapterInS3,
  createPresignedUrl,
  getChapterPrefixesFromS3,
  uploadFileToS3,
} from "./utils/aws";
import { buildPrefix, sortByPageNumber } from "./utils/helpers";
...
export const retrieveChapter = async (mangaId: string, chapterId: string) => {
  ...
  const s3prefixes = (
    exists
      ? await getChapterPrefixesFromS3(mangaId, chapterId)
  ...
  return s3prefixes.map(
    (prefix) =>
      `https://manga-reader-chapters.s3.us-west-2.amazonaws.com/${prefix}`
  );
};
